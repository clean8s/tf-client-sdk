...
import (
	...
	proto "github.com/clean8s/tf-client-sdk/tfplugin5"
	ctyjson "github.com/zclconf/go-cty/cty/json"
	"github.com/zclconf/go-cty/cty/msgpack"
	"google.golang.org/grpc"
	convert2 "github.com/clean8s/tf-client-sdk/plugin/convert"
...
func (p *GRPCProvider) GetProviderSchema() (resp providers.GetProviderSchemaResponse) {
	...
	resp.Diagnostics = resp.Diagnostics.Append(convert2.ProtoToDiagnostics(protoResp.Diagnostics))
	...
	resp.Provider = convert2.ProtoToProviderSchema(protoResp.Provider)
	if protoResp.ProviderMeta == nil {
		logger.Debug("No provider meta schema returned")
	} else {
		resp.ProviderMeta = convert2.ProtoToProviderSchema(protoResp.ProviderMeta)
	}

	for name, res := range protoResp.ResourceSchemas {
		resp.ResourceTypes[name] = convert2.ProtoToProviderSchema(res)
	}

	for name, data := range protoResp.DataSourceSchemas {
		resp.DataSources[name] = convert2.ProtoToProviderSchema(data)
	}
...
func (p *GRPCProvider) PlanResourceChange(r providers.PlanResourceChangeRequest) (resp providers.PlanResourceChangeResponse) {
	...
	for _, p := range protoResp.RequiresReplace {
		resp.RequiresReplace = append(resp.RequiresReplace, convert2.AttributePathToPath(p))
	}
